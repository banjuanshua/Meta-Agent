...
from models.classifier import ProtoHead, Agent
...
if __name__ == '__main__':
    ...
    agent = Agent(1, 5, 16000)
    agent.cuda()
    optimizer = torch.optim.SGD([{'params': embedding_net.parameters()}, 
                                 {'params': cls_head.parameters()},
                                 {'params': agent.parameters()}], lr=0.1, momentum=0.9, \
    ...
    for epoch in range(1, opt.num_epoch + 1):
        ...
        _, _, _ = [x.train() for x in (embedding_net, cls_head, agent)]
        ...
        for i, batch in enumerate(tqdm(dloader_train(epoch)), 1):
            ...
            logit_query = agent(emb_support, emb_query, labels_support,
            ...
            print('loss ', round(loss.item(), 3), 'acc', round(acc.item(), 3))
        ...
        _, _, _ = [x.eval() for x in (embedding_net, cls_head, agent)]
        ...
        for i, batch in enumerate(tqdm(dloader_val(epoch)), 1):
            ...
            print('val loss ', round(loss.item(), 3), 'val acc', round(acc.item(), 3))
